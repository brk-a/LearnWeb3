...
contract ToDoList {
    ...
    mapping (address => ToDoListApp) public addressToToDoListApp;
    ...
    function createList(string calldata _message) external {
        ...
        ToDoListApp storage toDo = addressToToDoListApp[msg.sender];
        ...
        emit ToDoEvent(toDo.account, toDo.userId, toDo.message, toDo.completed);
    }

    function getCreatorData(address _address)
        public view
        returns(address, uint256, string memory, bool) {
            ToDoListApp memory singleUserData = addressToToDoListApp[_address];
    ...
    function toggle(address _creator) public {
        ToDoListApp storage singleUserData = addressToToDoListApp[_creator];
